...
#-----------------------------------------------------------------------------
# dummy CorrectMap for catsoop

class CorrectMap:
    '''
    Store hint for each answer-id
    '''
    def __init__(self):
        self.the_hint = ""
        return
...
class HintSystem(object):
    ...
    def __init__(self, anum=0, hints=None, verbose_fail=False, 
        ...
        '''
        anum                 = (int) answer_id index number, to base hint off of
        hints                = (dict) hints to use (defaults to global variable "hints")
        ...
        color                = (str) color to use for hints
        '''
        ...
        # print("hint system initialized with hints=%s" % hints)
    ...
    def catsoop_check_hint(self, cfn):
        self.check_function = cfn
        return self.catsoop_check_function_wrapped

    def catsoop_check_function_wrapped(self, sub, soln):
        score, msg = self.check_function(sub, soln)
        if score:
            return score, msg

        print("catsoop_check_hint called, sub=%s" % sub)
        if score==1.0:
            return
        old_cmap = CorrectMap()
        new_cmap = CorrectMap()
        answer_ids = ["0"]
        self.check_hint(answer_ids, sub, new_cmap, old_cmap, anum=0)
        # msg = "[general_hint_system.catsoop_check_hint] hint=%s" % (new_cmap.the_hint)
        # print(msg)
        if (msg):
            msg += "<br/>"
        msg += new_cmap.the_hint
        if (msg):
            msg += "<br/>"
        try:
            ans = sub["0"]
            msg += "Your submission parsed as: <math>%s</math>" % latex_preview(ans)
        except Exception as err:
            print("[general_hint_system.catsoop_check_hint] could not generate preview, sub=%s, err=%s" % (sub, err))
        return score, msg
    ...
    def check_hint(self, answer_ids, student_answers, new_cmap, old_cmap, anum=None, the_hints=None):
        ...
        # print("[ghs.check_hint] ans=%s" % ans)
